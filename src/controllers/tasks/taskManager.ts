...
const TaskDetail = async ({ id }, token: string) => {
  try {
    ...
    if (!detail) {
      return { statusCode: 404, message: "Task not found", detail: null };
    }

    const userId = await LoadUserInfo(token).userId;
    const relatedUserIds = [detail?.authorId, detail?.employeeId];
    const isUserRelated = relatedUserIds.includes(userId);
...
const UpdateTask = async (
  ...
  try {
    ...
    const priorityData = HandlePriority({
      priority: priority || EnumData.PriorityType.Low.code,
    });

    const userInfo = LoadUserInfo(token);

    const getUserId = await prisma.tasks.findFirst({
      where: { id },
      select: { authorId: true, employeeId: true },
    })

    const relatedUserIds = [getUserId?.authorId, getUserId?.employeeId];
    const isUserRelated = relatedUserIds.includes(userInfo?.userId);
